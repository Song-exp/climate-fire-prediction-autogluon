...
for i in range(len(file_name)):
    ...
    csv_file_path = f'C:/빅데이터 활용 미래 사회문제 해결 아이디어 해커톤/모델결과_결과값cat/result_{file_name[i]}'
    result_df.to_csv(csv_file_path, index=False)   

    model_result = pd.DataFrame({'Performance': [performance],
                                 'Model': [model]})
    
    csv_file_path1 = f'C:/빅데이터 활용 미래 사회문제 해결 아이디어 해커톤/모델결과_모델정보cat/model_{file_name[i]}'
