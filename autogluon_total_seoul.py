...
df_seoul = pd.read_csv('C:/빅데이터 활용 미래 사회문제 해결 아이디어 해커톤/전처리 후 데이터/서울.csv', index_col=0)
...
# df_model['화재합계'] = df_model['화재합계'].map(fire_cat_dict)
    ...
    # 클래스 불균형 문제를 확인하고 처리합니다.
    unique_classes = df_train[y].unique()
    if len(unique_classes) > 1:
        # 데이터셋에 두 개 이상의 클래스가 있는 경우, 모델을 훈련하고 평가합니다.
        train_data = TabularDataset(df_train)
        test_data = TabularDataset(df_test)
        predictor = TabularPredictor(label=y, problem_type='regression', eval_metric='root_mean_squared_error').fit(train_data=train_data, presets='best_quality')

        # predictor = TabularPredictor(label=y).fit(train_data=train_data, verbosity = 2, presets='best_quality')
        performance = predictor.evaluate(test_data)
        predictions = predictor.predict(test_data)
        model = predictor.get_model_best() # 또는 predictor.model_type
    else:
        # 데이터셋에 클래스가 하나만 있는 경우, 경고를 출력하고 이번 반복을 건너뜁니다.
        print(f"Skipping {y}: Only one class present in training data.")
        continue
    ...
    # predictor= TabularPredictor(label = y).fit(train_data = train, verbosity = 2,presets='best_quality')
    # performance = predictor.evaluate(test)
    # predictions = predictor.predict(test) # 새로운 데이터 셋에 대한 예측값 생성
    

    result_df = pd.DataFrame({'observation' : test_data[y],
                                'Predictions': predictions})

    csv_file_path = f'C:/빅데이터 활용 미래 사회문제 해결 아이디어 해커톤/모델결과_seoul/result_{y}'
    result_df.to_csv(csv_file_path, index=False)   

    model_result = pd.DataFrame({'Performance': [performance],
                                    'Model': [model]})

    csv_file_path1 = f'C:/빅데이터 활용 미래 사회문제 해결 아이디어 해커톤/모델결과_seoul/model_{y}'
